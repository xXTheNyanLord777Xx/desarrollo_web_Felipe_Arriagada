...
def get_aviso_detail(aviso_id):
    ...
    #ACA LO NUEVOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO}
    #tenemos que añadir la opcion para añadir comentarios y mostrarlos, entonces
    #debemos obtenerlas primero, en db.py definiremos unas funciones para los comentarios

    comentarios_list = db.listado_comentario(aviso_id)
...
#T3
#estadisticas lo llamamos
@app.route("/estadisticas", methods=["GET"])
def stats():
    return render_template("estadisticas.html")
#ahora debemos crear a getstatsdata, del tipo API, para que nos retorne un json 
@app.route("/get-stats-data", methods=["GET"])
#sigue el CROSS JOIN
@cross_origin(origin="127.0.0.1", supports_credentials=True)
def stats_por_dia():
    #aca debemos poner la data que generamos
    #retornando en JSON :o
    #los obtenemos con un query por lo tanto llamaremos a la funcion que esta en db
    return jsonify(db.stats_dia())

#hay que hacerlo para cada funcion alv, asi que ahora con perros y gatos
@app.route("/get-stats-tipo", methods=["GET"])
#sigue el CROSS JOIN
@cross_origin(origin="127.0.0.1", supports_credentials=True)
def perro_gato():
    return jsonify(db.perrogato())

@app.route("/get-stats-mestipo", methods=["GET"])
#sigue el CROSS JOIN
@cross_origin(origin="127.0.0.1", supports_credentials=True)
def tipo_por_mes():
    return jsonify(db.mestipo())


#ahora hay q poner la api para enviar el comentario del modal
@app.route("/api/aviso/<int:aviso_id>/comentario", methods=["POST"])
def agregar_comentario(aviso_id):
    #obtenemos nombre y texto
    nombre = request.form.get("nombre")
    texto  = request.form.get("texto")
    
    ok, errores = db.subir_comentario(aviso_id, nombre, texto)
    if not ok:
        #retornamos la lista de errores
        return jsonify({"ok": False, "errors": errores}), 400

    #devolvemos el comentario
    return jsonify({"ok": True, "comentario": errores}), 201
