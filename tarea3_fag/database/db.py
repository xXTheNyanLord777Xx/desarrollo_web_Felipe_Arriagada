...
from collections import Counter, defaultdict #para contar los avisos
DB_NAME = "tarea2"
DB_USERNAME = "cc5002"
...
#añadimos la clase de comentario yuju
class Comentario(Base):
    ...
    fecha = Column(DateTime, default=datetime.utcnow, nullable=False) #fecha de publicacion
...
def stats_dia():
    session = SessionLocal()
    dia = session.query(AvisoAdopcion.fecha_ingreso).all()
    #ahora tambien debemos contarlos, para ello generamos un dict donde este la fecha
    #y luego el count, que lo haremos utilizando count de collections

    c = Counter()#contador vacio
    for (ts,) in dia:#recorremos las TUPLAS y sumamos 1 a la fecha del DIA, no nos importa la hora
        #con ts, podemos asegurarnos de estar accediendo al primer valor
        c[ts.date()] += 1

    data = [{
        "date": d.isoformat(),#date es datetime, con isoformat lo dejamos en string para que json lo reciba bien
        "count": c[d]#coun tendra el valor que fue agregado el counter
    }for d in sorted(c.keys())]#con sorted ordenamos las FECHAS, de menor a mayor
    return data


def perrogato():
    session = SessionLocal()
    animales = session.query(AvisoAdopcion.tipo).all()
    c = Counter(t for (t,) in animales) #esto es para contar la cantidad de perros y gatos
    #entonces en data solo añadimos la cuenta que se hizo en el counter
    data = [
            {"name": "Gato", "y": c.get("gato", 0)},
            {"name": "Perro", "y": c.get("perro", 0)}
    ]
...
def mestipo():
    session = SessionLocal()
    filas = session.query(AvisoAdopcion.fecha_ingreso, AvisoAdopcion.tipo).all()

    conteo = defaultdict(lambda: {"gato": 0, "perro": 0})

    for fecha, tipo in filas:
        # Formateamos la fecha como "YYYY-MM"
        mes_str = f"{fecha.year:04d}-{fecha.month:02d}"

        # Sumamos 1 al tipo correspondiente
        if tipo.lower() == "gato":
            conteo[mes_str]["gato"] += 1
        elif tipo.lower() == "perro":
            conteo[mes_str]["perro"] += 1

        # Convertimos el dict en una lista ordenada por mes
    data = [
        {"month": mes, "gato": datos["gato"], "perro": datos["perro"]}
        for mes, datos in sorted(conteo.items())
    ]

    return data


#AHORA HACEMOS LAS FUNCIONES PARA LOS COMENTARIOOOOS

#funcion que sube el comentario, recibe el aviso, quien lo subio y su contenido
def subir_comentario(aviso_id: int, nombre:str,texto:str):
    #igual que siempre creamos lista de errores y si no cumple el largo o lo que sea
    #se mete el error a la lista, quizas debio ser en validations pero ya fue

    #el nombre me esta dando none alv
    #nombre = (nombre or "").strip()
    #texto  = (texto  or "").strip()

    errores = []
    if len(nombre)<3 or len(nombre) > 80:
        errores.append("Nombre debe ser entre 3 y 80 caracteres")
    if len(texto) < 5:
        errores.append("El comentario debe tener como minimo 5 caracteres")
    #si no cumplen esos erroes
    if errores:
        return False, errores
    
    session = SessionLocal()

    new_comentario = Comentario(aviso_id=aviso_id,nombre=nombre,texto=texto,)
    session.add(new_comentario)
    session.commit()
    #retornamos que funciono(TRUE) y el aviso a añadir
    return True, {
...
#aca vemos los comentarios que tiene la publicacion segun el id del aviso
def listado_comentario(aviso_id: int):
    session = SessionLocal()
    #accedemos a las filas de comentarios que podemos tener
    Comentarios = (
    ...
    #ahora revisamos sus datos 1 a 1 con un for y retornamos la lista
    L = []
